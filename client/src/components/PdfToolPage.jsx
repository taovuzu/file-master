...
const PdfToolPage = ({
  ...
  const handleProcess = async (formValues = {}) => {
    if (!fileList || fileList.length === 0) {
      message.warning(`Please upload ${multipleFiles ? 'at least one' : 'a'} PDF file first.`);
      return;
    }
  ...
  const handleDownload = () => {
    if (processedFile) {
      ...
      link.download = downloadFileName || `processed_${fileList[0]?.name || 'document.pdf'}`;
            ...
            {showPreview && (
              <>
                ...
                {previewLoading ? (
                  ...
                  <PdfPreview files={multipleFiles ? fileList : [fileList[0]]} />
                )}
              </>
            )}
